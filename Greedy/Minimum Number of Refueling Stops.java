...
class Solution {
    public int minRefuelStops(int target, int startFuel, int[][] stations) {
        int stops=0; int CanTravelUpTo = startFuel;
        
        PriorityQueue<Integer> maxHeap = new PriorityQueue<>((a,b)->b-a);
        
        for(int[] s: stations){
            
            //s[0]-> station location
            //if i cannot reach the location, is when we add fuel and check whether we can reach or not
            while(CanTravelUpTo<s[0]){
                
                //if heap doesn't have anything, that means we cannot fuel and go
                if(maxHeap.isEmpty())
                    return -1;
                
                stops++;
                
                //after adding fuel, our canTravelUpto will increase, so we can check whether we can reach the current station location
        ...
        //above we visited all stations
        //now lets check whether we can reach target location
