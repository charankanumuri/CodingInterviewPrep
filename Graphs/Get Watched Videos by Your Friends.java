...
class Solution {
    public List<String> watchedVideosByFriends(List<List<String>> watchedVideos, 
        ...
        //gets friends of a particular level
        while(level>0){
            int size = queue.size();
            
            for(int i=0;i<size;i++){
                Integer friend_id = queue.poll();
                
                for(Integer friend: friends[friend_id]){
                    if(!visited[friend]){
                        queue.add(friend);
                        visited[friend]=true;
                    }
        ...
        //get list of movies watched by friends at that level as per the question
        while(!queue.isEmpty()){
            Integer friend_id = queue.poll();
            List<String> movies = watchedVideos.get(friend_id);
            for(String movie: movies){
                movieFreq.put(movie, movieFreq.getOrDefault(movie, 0)+1);
            }
        }
        
        //create result array with movies watched from the map
        List<String> result = new ArrayList<>(movieFreq.keySet());
        
        //sort values based on lesser frequency
        result.sort((a,b)-> {
                   Integer one = movieFreq.get(a);
                   Integer two = movieFreq.get(b);
                    
                    if(one!=two)
                        return one-two;
                    else
                        return a.compareTo(b); //if not take lexographically which comes first
        });
