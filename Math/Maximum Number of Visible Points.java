...
class Solution {
    public int visiblePoints(List<List<Integer>> points, int angle, List<Integer> location) {
        ...
        int maxPoints = 0, lX = location.get(0), lY = location.get(1);
        int same = 0;
        ...
        for(List<Integer> p: points){
            ...
            if((lX-x==0) && (lY-y==0))
                ...
                angles.add(Math.atan2(y-lY, x-lX) * (180/Math.PI));
        ...
        //adding 360 to add angles to handle negative angles
        List<Double> temp = new ArrayList<>(angles);
        for(int i=0;i<angles.size();i++)
            temp.add(angles.get(i)+360);
        
        int left=0;
        for(int right = 0;right<temp.size();right++){
            
            while(temp.get(right)-temp.get(left)>angle)
                left++;
